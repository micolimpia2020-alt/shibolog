...
// ─── 今日基準で今週を生成する ─────────────────────────
// 【修正①】サンプルデータを完全に削除し、今日の日付から週を自動生成する
...
function makeEmptyWeek(monday) {
  ...
  return { weekLabel: label, reflection: "", trainerComment: "", days };
}
...
function buildInitialWeeks() {
  ...
  return [
    makeEmptyWeek(lastMonday),
    makeEmptyWeek(thisMonday),
  ];
}
...
export default function DietTracker() {
  ...
  const [profile, setProfile] = useState(() => {
    try {
      const s = localStorage.getItem("shibolog_profile");
      return s ? JSON.parse(s) : { height:"", weight:"", fatPct:"", maintenance:"" };
    } catch { return { height:"", weight:"", fatPct:"", maintenance:"" }; }
  });

  // 【修正②】weeksの初期値：保存データがあればそれを使い、なければ今日基準の空データを生成
  const [weeks, setWeeks] = useState(() => {
    try {
      const s = localStorage.getItem("shibolog_weeks");
      if (s) {
        const saved = JSON.parse(s);
        // 保存データが存在する場合、今週のデータがなければ追加する
        const today = new Date();
        const thisMonday = getMonday(today);
        const thisWeekLabel = makeWeekLabel(thisMonday);
        const hasThisWeek = saved.some(w => w.weekLabel === thisWeekLabel);
        if (!hasThisWeek) {
          return [...saved, makeEmptyWeek(thisMonday)];
        }
        return saved;
      }
  ...
  // 自動保存
  useEffect(() => {
    try { localStorage.setItem("shibolog_profile", JSON.stringify(profile)); } catch {}
  }, [profile]);
  useEffect(() => {
    try { localStorage.setItem("shibolog_weeks", JSON.stringify(weeks)); } catch {}
  }, [weeks]);

  // 【修正③】デフォルトで今週（最新週）を選択する
  const [selWeek, setSelWeek] = useState(() => {
    const today = new Date();
    const thisMonday = getMonday(today);
    const thisWeekLabel = makeWeekLabel(thisMonday);
    const idx = weeks.findIndex(w => w.weekLabel === thisWeekLabel);
    return idx >= 0 ? idx : weeks.length - 1;
  });

  // 【修正④】今日の曜日インデックスを自動選択（月=0,火=1...日=6）
  const [selDay, setSelDay] = useState(() => {
    const today = new Date();
    const day = today.getDay();
    return day === 0 ? 6 : day - 1; // 日曜=6, 月曜=0
  });
  ...
  const [goal, setGoal] = useState(() => {
    try {
      const s = localStorage.getItem("shibolog_goal");
  ...
  useEffect(() => {
    try { localStorage.setItem("shibolog_goal", JSON.stringify(goal)); } catch {}
  }, [goal]);
  ...
  async function setupReminders() {
    ...
    if(perm !== "granted") { setRemindStatus("通知が許可されませんでした。ブラウザの設定から許可してください。"); return; }
    setRemindEnabled(true);
    setRemindStatus("✅ リマインドを設定しました！");
    scheduleNotification(remindMorning, "📝 記録の時間です！", "今日の記録は済みましたか？日々の記録が目標達成の近道です✨");
    scheduleNotification(remindNight, "🌙 お疲れ様でした！", "今日もお疲れ様でした✨ 夜の記録を忘れずに。明日も頑張りましょう！");
  }
  ...
  function scheduleNotification(timeStr, title, body) {
    ...
    setTimeout(() => { new Notification(title, { body, icon:"🔥" }); }, delay);
  }
      ...
      <div style={{padding:"16px 16px 0"}}>
          ...
          <div style={{display:"flex",flexDirection:"column",gap:14}}>
            ...
            {(goal.type || goal.targetNum || goal.targetDate) && (
              <div style={{display:"flex",alignItems:"center",gap:10,padding:"10px 14px",borderRadius:12,background:`linear-gradient(135deg,rgba(255,107,157,0.12),rgba(180,138,255,0.1))`,border:`1px solid ${C.pink}44`}}>
                ...
                <button onClick={()=>setTab("設定")} style={{fontSize:10,color:C.purple,background:"none",border:"none",cursor:"pointer",fontWeight:700,whiteSpace:"nowrap"}}>編集 →</button>
              </div>
            )}
                  ...
                  return (
                    <div key={i} style={{background:hasTrain?"rgba(255,107,53,0.12)":"rgba(255,255,255,0.04)",borderRadius:12,border:hasTrain?`1px solid ${C.orange}44`:`1px solid ${C.border}`,padding:"8px 4px",textAlign:"center",minHeight:90,display:"flex",flexDirection:"column",alignItems:"center",gap:3}}>
                      ...
                      {parts.slice(0,2).map((p,pi) => (
                        <div key={pi} style={{fontSize:8,fontWeight:700,background:`${C.orange}22`,color:C.orange,borderRadius:4,padding:"1px 4px",maxWidth:"100%",overflow:"hidden",textOverflow:"ellipsis",whiteSpace:"nowrap"}}>{p}</div>
                      ))}
        ...
        {tab==="食事" && (
          <div style={{display:"flex",flexDirection:"column",gap:14}}>
            ...
            {praiseMsg && (
              <div style={{padding:"14px 16px",borderRadius:14,background:`linear-gradient(135deg,rgba(255,107,53,0.18),rgba(180,138,255,0.18))`,border:`1px solid ${C.orange}55`,fontSize:14,fontWeight:700,lineHeight:1.6,animation:"fadeIn 0.3s ease"}}>
                {praiseMsg}
              </div>
            )}
              ...
              {bodyPhoto ? (
                <div>
                  ...
                  <div style={{display:"grid",gridTemplateColumns:"1fr 1fr",gap:8}}>
                    <button onClick={()=>{bodyPhotoRef.current.removeAttribute("capture");bodyPhotoRef.current.click();}} style={{padding:"8px",border:`1px solid ${C.border}`,borderRadius:10,background:"rgba(255,255,255,0.04)",color:C.muted,fontSize:12,cursor:"pointer"}}>🔄 写真を変更</button>
                ...
                <div style={{display:"grid",gridTemplateColumns:"1fr 1fr",gap:8}}>
                  <button onClick={()=>{bodyPhotoRef.current.removeAttribute("capture");bodyPhotoRef.current.click();}} style={{...btnGrad(C.pink,C.purple),padding:"11px",fontSize:12}}>📁 フォルダから選択</button>
                  <button onClick={()=>{bodyPhotoRef.current.setAttribute("capture","environment");bodyPhotoRef.current.click();}} style={{...btnGrad(C.purple,C.blue),padding:"11px",fontSize:12}}>📷 カメラで撮影</button>
                </div>
              )}
            </div>
            ...
            <div style={cardSt()}>
              ...
              <div style={{marginBottom:10}}>
                ...
                <div style={{display:"grid",gridTemplateColumns:"1fr 1fr",gap:8}}>
                  <button onClick={()=>{photoRef.current.removeAttribute("capture");photoRef.current.click();}} style={{padding:"9px",border:`1px solid ${C.border}`,borderRadius:10,background:"rgba(255,255,255,0.04)",color:C.muted,fontSize:12,cursor:"pointer"}}>📁 フォルダから選択</button>
                  <button onClick={()=>{photoRef.current.setAttribute("capture","environment");photoRef.current.click();}} style={{padding:"9px",border:`1px solid ${C.border}`,borderRadius:10,background:"rgba(255,255,255,0.04)",color:C.muted,fontSize:12,cursor:"pointer"}}>📷 カメラで撮影</button>
                </div>
                    ...
                    <div style={{display:"flex",gap:6,marginBottom:8,flexWrap:"wrap"}}>
                      {["😊","😆","😍","😭","😡","🤤"].map(emoji => (
                        <button key={emoji} onClick={()=>updateDay(selWeek,realIdx,{mood:d.mood===emoji?"":emoji})}
                          style={{fontSize:20,background:d.mood===emoji?"rgba(255,107,53,0.25)":"rgba(255,255,255,0.06)",border:d.mood===emoji?`1px solid ${C.orange}`:`1px solid ${C.border}`,borderRadius:10,padding:"4px 8px",cursor:"pointer",transform:d.mood===emoji?"scale(1.2)":"scale(1)",transition:"all 0.15s"}}>
                          {emoji}
                        </button>
                      ))}
                    </div>
          ...
          <div style={{display:"flex",flexDirection:"column",gap:14}}>
            ...
            <div style={cardSt({border:`1px solid ${C.pink}55`,background:`linear-gradient(135deg,rgba(255,107,157,0.08),rgba(180,138,255,0.06))`})}>
                ...
                <div style={{display:"flex",flexWrap:"wrap",gap:8}}>
                  {["ダイエット","筋力アップ","絞る","健康維持","その他"].map(t => (
                    <button key={t} onClick={()=>setGoal(g=>({...g,type:t}))}
                      style={{padding:"7px 16px",borderRadius:999,border:"none",cursor:"pointer",fontSize:12,fontWeight:700,background:goal.type===t?`linear-gradient(135deg,${C.pink},${C.purple})`:"rgba(255,255,255,0.08)",color:goal.type===t?"#fff":C.muted}}>
                      {t}
                    </button>
                  ))}
                </div>
              ...
              <div style={{marginBottom:12}}>
                ...
                <input type="text" placeholder="例：体重45kgにする、体脂肪率15%にする" value={goal.targetNum} style={inpSt(`${C.pink}44`)} onChange={e=>setGoal(g=>({...g,targetNum:e.target.value}))}/>
              </div>
              <div style={{marginBottom:12}}>
                <div style={{fontSize:11,color:C.muted,marginBottom:5,fontWeight:600}}>✨ 見た目の目標</div>
                <textarea placeholder="例：腹筋を割りたい、二の腕を細くしたい" value={goal.targetLook} onChange={e=>setGoal(g=>({...g,targetLook:e.target.value}))} style={{...inpSt(`${C.purple}44`),height:64,resize:"none"}}/>
              </div>
              <div style={{marginBottom:12}}>
                ...
                {goal.refPhoto
                  ? <div>
                      ...
                      <button onClick={()=>setGoal(g=>({...g,refPhoto:null}))} style={{padding:"7px 16px",border:`1px solid ${C.red}44`,borderRadius:8,background:"rgba(255,95,95,0.07)",color:C.red,fontSize:12,cursor:"pointer"}}>🗑️ 削除</button>
                    </div>
              ...
              <div>
                ...
                <input type="text" placeholder="例：2025年8月、3ヶ月後" value={goal.targetDate} style={inpSt(`${C.yellow}44`)} onChange={e=>setGoal(g=>({...g,targetDate:e.target.value}))}/>
              </div>
            </div>
            ...
            <div style={cardSt({border:`1px solid ${C.teal}44`})}>
              ...
              {remindStatus && (
                <div style={{fontSize:12,padding:"8px 12px",borderRadius:8,background:remindStatus.startsWith("✅")?"rgba(74,222,128,0.1)":"rgba(255,95,95,0.1)",color:remindStatus.startsWith("✅")?C.green:C.red}}>
                  {remindStatus}
                </div>
              )}
              <div style={{fontSize:11,color:C.muted,marginTop:8}}>※ ブラウザを開いている間のみ通知されます。</div>
            </div>
  ...
  return (
    <div style={cardSt2()}>
      ...
      <div style={{display:"flex",justifyContent:"space-between",alignItems:"center",padding:"12px 14px",borderRadius:12,border:`1px solid ${C2.teal}44`,marginBottom:10,background:"rgba(255,255,255,0.03)"}}>
        ...
        <div style={{fontSize:22,fontWeight:800,color:C2.teal}}>{fmt0(maintenance)}<span style={{fontSize:11,color:C2.muted,marginLeft:2}}>kcal</span></div>
      </div>
      <div style={{padding:"12px 14px",borderRadius:12,border:`1px solid ${C2.orange}44`,marginBottom:10,background:"rgba(255,255,255,0.03)"}}>
        <div style={{display:"flex",justifyContent:"space-between",alignItems:"center",marginBottom:8}}>
          <div>
            ...
            {cutDiff != null && <div style={{fontSize:11,color:cutDiff<0?C2.orange:"#f87171",marginTop:2}}>メンテより {cutDiff>0?"+":""}{cutDiff} kcal</div>}
          </div>
          {cutVal != null && <div style={{fontSize:22,fontWeight:800,color:C2.orange}}>{cutVal}<span style={{fontSize:11,color:C2.muted,marginLeft:2}}>kcal</span></div>}
        </div>
        <input type="number" placeholder="目標カロリーを入力（例: 1000）" value={cutInput} style={inpSt2(`${C2.orange}44`)} onChange={e=>setCutInput(e.target.value)}/>
      </div>
      <div style={{padding:"12px 14px",borderRadius:12,border:`1px solid ${C2.green}44`,background:"rgba(255,255,255,0.03)"}}>
        <div style={{display:"flex",justifyContent:"space-between",alignItems:"center",marginBottom:8}}>
          <div>
            ...
            {bulkDiff != null && <div style={{fontSize:11,color:bulkDiff>0?C2.green:"#f87171",marginTop:2}}>メンテより {bulkDiff>0?"+":""}{bulkDiff} kcal</div>}
          </div>
          {bulkVal != null && <div style={{fontSize:22,fontWeight:800,color:C2.green}}>{bulkVal}<span style={{fontSize:11,color:C2.muted,marginLeft:2}}>kcal</span></div>}
        </div>
        <input type="number" placeholder="目標カロリーを入力（例: 1800）" value={bulkInput} style={inpSt2(`${C2.green}44`)} onChange={e=>setBulkInput(e.target.value)}/>
      </div>
    </div>
  );
}
...
function TrainingTab({ weeks, currentWeek, currentDay, selWeek, selDay, updateDay, showPraise, C, cardSt, inpSt, btnGrad, setSelDay }) {
  ...
  function addExercise() {
    ...
    updateDay(selWeek, selDay, { training:{...training, exercises:[...exercises,ex], done:true} });
    setExName("");
  ...
  function removeExercise(id) {
    updateDay(selWeek, selDay, { training:{...training, exercises:exercises.filter(e=>e.id!==id)} });
  }
    ...
    <div style={{display:"flex",flexDirection:"column",gap:14}}>
      ...
      <div style={cardSt()}>
        <div style={{display:"flex",alignItems:"center",gap:12,marginBottom:14}}>
          ...
          }} style={{marginLeft:"auto",padding:"7px 18px",border:"none",borderRadius:999,cursor:"pointer",fontWeight:700,fontSize:12,background:training.done?`linear-gradient(135deg,${C.green},${C.teal})`:"rgba(255,255,255,0.07)",color:training.done?"#000":C.muted}}>
            ...
            {PARTS.map(part => {
              ...
              return (
                <button key={part} onClick={()=>togglePart(part)}
                  style={{padding:"6px 13px",border:"none",borderRadius:999,cursor:"pointer",fontSize:12,fontWeight:700,background:active?`linear-gradient(135deg,${C.orange},${C.pink})`:"rgba(255,255,255,0.07)",color:active?"#fff":C.muted}}>
                  {part}
                </button>
              );
            })}
      ...
      <div style={cardSt({border:`1px solid ${C.orange}44`})}>
        ...
        <div style={{marginBottom:14}}>
          ...
          <input type="text" placeholder="例: ベンチプレス、スクワット、デッドリフト" value={exName} style={inpSt()} onChange={e=>setExName(e.target.value)}/>
        </div>
        {prevExercise && (
          <div style={{marginBottom:12,padding:"10px 12px",borderRadius:10,background:"rgba(94,174,255,0.07)",border:`1px solid ${C.blue}33`}}>
            ...
            <div style={{display:"flex",gap:6,flexWrap:"wrap"}}>
              {prevExercise.sets.map((s,si) => (
                <div key={si} style={{fontSize:11,padding:"3px 9px",borderRadius:999,background:`${C.blue}18`,color:C.blue,fontWeight:600}}>
                  Set{s.setNo} {s.weight}kg×{s.reps}回{s.rm&&<span style={{color:C.muted,marginLeft:3}}>1RM:{s.rm}kg</span>}
                </div>
              ))}
            </div>
            <div style={{fontSize:10,color:C.muted,marginTop:4}}>前回最高1RM：<span style={{color:C.orange,fontWeight:700}}>{prevExercise.bestRM}kg</span></div>
          </div>
        )}
        ...
        <button onClick={addExercise} disabled={!exName||filledSets.length===0}
          style={{...btnGrad(C.orange,C.pink),width:"100%",padding:"13px",fontSize:14,opacity:(!exName||filledSets.length===0)?0.4:1}}>
          ＋ この種目を記録する
        </button>
      </div>
          ...
          <div key={i} style={{padding:"10px 0",borderBottom:`1px solid ${C.border}`}}>
            <div style={{display:"flex",justifyContent:"space-between",alignItems:"center",marginBottom:d.training?.exercises?.length?6:0}}>
              ...
              <div style={{fontSize:12,display:"flex",gap:8}}>
                {d.training?.done?<span style={{color:C.green}}>💪{d.training.parts||"筋トレ"}</span>:<span style={{color:C.muted}}>休養日</span>}
                {d.training?.exercises?.length>0&&<span style={{color:C.teal}}>{d.training.exercises.length}種目</span>}
                {d.training?.cardio>0&&<span style={{color:C.blue}}>🏃{d.training.cardio}分</span>}
              </div>
            </div>
            {d.training?.exercises?.length > 0 && (
              <div style={{display:"flex",flexWrap:"wrap",gap:6}}>
                {d.training.exercises.map((ex,ei) => (
                  <span key={ei} style={{fontSize:11,padding:"2px 10px",borderRadius:999,background:`${C.orange}18`,color:C.orange,fontWeight:600}}>
                    {ex.name}{ex.bestRM>0&&<span style={{color:C.muted,marginLeft:3}}>1RM {ex.bestRM}kg</span>}
                  </span>
                ))}
              </div>
            )}
...
function CalendarTab({ weeks, updateDay, setTab, C, cardSt, inpSt, btnGrad }) {
  ...
  function setWeight(field, value) {
    if(!selectedData) return;
    updateDay(selectedData.weekIdx, selectedData.dayIdx, {
    ...
    <div style={{display:"flex",flexDirection:"column",gap:14}}>
      <div style={cardSt()}>
        ...
        <div style={{display:"grid",gridTemplateColumns:"repeat(7,1fr)",gap:2,marginBottom:4}}>
          {DOW.map(d => (
            <div key={d} style={{textAlign:"center",fontSize:10,fontWeight:700,color:d==="日"?C.red:d==="土"?C.blue:C.muted,padding:"4px 0"}}>{d}</div>
          ))}
        </div>
        {/* 日付グリッド */}
          ...
          {Array.from({length:weeksCount*7}).map((_,idx) => {
            ...
            return (
              <div key={idx} onClick={()=>setSelected(isSelected?null:dateKey)}
                style={{minHeight:56,borderRadius:10,padding:"4px 3px",cursor:"pointer",textAlign:"center",background:isSelected?`linear-gradient(135deg,${C.purple}44,${C.blue}33)`:hasTrain?"rgba(255,107,53,0.1)":"rgba(255,255,255,0.04)",border:isSelected?`1.5px solid ${C.purple}`:isToday?`1.5px solid ${C.teal}`:`1px solid rgba(255,255,255,0.09)`,display:"flex",flexDirection:"column",alignItems:"center",gap:1}}>
      ...
      {selected && (
        <div style={cardSt({border:`1px solid ${C.purple}44`})}>
          ...
          {/* 【追加】体重入力フォーム */}
          <div style={{marginBottom:14}}>
            <div style={{fontSize:11,color:C.yellow,fontWeight:700,marginBottom:8}}>⚖️ 体重を記録</div>
            <div style={{display:"grid",gridTemplateColumns:"1fr 1fr",gap:10}}>
              <div>
                <div style={{fontSize:11,color:C.yellow,marginBottom:5}}>🌅 朝の体重 (kg)</div>
                <input
                  type="number" step="0.1" placeholder="例: 48.5"
                  value={selectedData?.morning ?? ""}
                  style={inpSt(`${C.yellow}55`)}
                  onChange={e => setWeight("morning", e.target.value)}
                />
              </div>
              <div>
                <div style={{fontSize:11,color:C.purple,marginBottom:5}}>🌙 夜の体重 (kg)</div>
                <input
                  type="number" step="0.1" placeholder="例: 49.0"
                  value={selectedData?.night ?? ""}
                  style={inpSt(`${C.purple}55`)}
                  onChange={e => setWeight("night", e.target.value)}
                />
              </div>
            </div>
          </div>
          ...
          {/* 気分 */}
          <div style={{marginBottom:12}}>
            <div style={{fontSize:11,color:C.muted,marginBottom:6}}>気分・表情</div>
            <div style={{display:"flex",gap:8,flexWrap:"wrap"}}>
              {MOODS.map(emoji => (
                <button key={emoji} onClick={()=>setMood(emoji)}
                  style={{fontSize:20,background:selectedData?.mood===emoji?"rgba(180,138,255,0.25)":"rgba(255,255,255,0.06)",border:selectedData?.mood===emoji?`1px solid ${C.purple}`:`1px solid rgba(255,255,255,0.09)`,borderRadius:10,padding:"4px 8px",cursor:"pointer"}}>
                  {emoji}
                </button>
              ))}
            </div>
          ...
          <div>
            ...
            <div style={{display:"flex",gap:8}}>
              ...
              <button onClick={saveSchedule} style={{...btnGrad(C.teal,C.blue),padding:"10px 16px",fontSize:13,borderRadius:10,whiteSpace:"nowrap"}}>保存</button>
            </div>
          </div>
        </div>
      )}
